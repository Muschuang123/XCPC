...
template <class T>              // a x b 从a看b，b在顺时针方向是负，逆时针方向是正 (rad < 90)
...
// 旋转 90°
template <class T>
Point<T> rotate(Point<T> a, Point<T> b)
{ // 旋转
    Point<T> vec = a - b;
    return {-vec.y, vec.x};
}
...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        int x, y;
        cin >> x >> y;
        // x = rnd() % 998244353, y = rnd() % 998244353;
        vis[x][y] = 1;
        a[i].x = x, a[i].y = y;
    }
    int ans = 0;
        ...
        for (int j = 1; j <= n; j++)
        {
            ...
            for (int k = 1; k <= n; k++)
            {
                ...
                if (onLine(a[i], a[j], a[k]))
                    continue;
                Point<ld> tmp = (a[i] + a[j]) / 2;
                if (tmp == a[k])
                    continue;
                Line<ld> l(tmp, a[k]);
                auto [A, B, C] = getfun(l);
                auto nC = A * a[i].x + B * a[i].y;
                // cout << a[i] << a[j] << tmp << endl;
                // cout << A << ' ' << B << endl;
                Line<ld> l1 = getfun(A, B, nC);

                auto nC2 = B * a[i].x - A * a[i].y;
                // auto ans = a[i] - rotate(a[i], );
                // Line<ld> l2(a[i], ans);
                Line<ld> l2 = getfun(B, -A, nC2);
                // cout << l << l1 << l2 << endl;

                Pd t1 = project(a[j], l1);
                Point<ll> nt1(t1.x,t1.y);
                Pd t2 = project(a[j], l2);
                Point<ll> nt2(t2.x,t2.y);
                Pd t = project(a[j], l);

                if (!equal(nt1.x, t1.x) || !equal(nt1.y , t1.y) || !equal(nt2.x , t2.x) || !equal(nt2.y , t2.y))
                    continue;
                if (!vis.count(nt1.x) || !vis[nt1.x].count(nt1.y))
                    continue;
                if (!vis.count(nt2.x) || !vis[nt2.x].count(nt2.y))
                    continue;
                if (t1 == t2 || t1 == a[i] || t1 == a[j] || t2 == a[i] || t2 == a[j])
                    continue;
                if (!pointOnSegment(t, l) || disEx(a[j], a[k]) != disEx(t1, a[k]))
                    continue;
                if (onLine(a[k], t1, t2) || onLine(a[k], t1, a[i]) || onLine(a[k], t1, a[j]) || onLine(a[k], t2, a[i]) || onLine(a[k], t2, a[j]))
                    continue;
                ans++;
            }
        }
