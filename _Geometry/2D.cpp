...
struct Polygon {
    ...
    T area() const {
        T sum = 0;
        for (int i = 0; i < p.size(); i++) sum += p[i] ^ p[nxt(i)];
        return sum;
    }
    // 多边形的周长
    T circ() const {
        T sum = 0;
        for (int i = 0; i < p.size(); i++) sum += p[i].dis(p[nxt(i)]);
        return sum;
    }
};
...
struct Convex : Polygon {
    ...
    Convex operator+(const Convex &c) const {
        ...
        for (int i = 0; i < p.size(); i++) e1[i] = {p[i], p[this->nxt(i)]};
    ...
    int extreme(const F &dir) const {
        const auto &p = this->p;
        const auto check = [&](const int i) { return dir(p[i]).toleft(p[this->nxt(i)] - p[i]) >= 0; };
