...
struct Line {
    ...
    // i64 + pair<ld, ld>
    // pair<ld, ld> inter(const Line &a) const {
    ...
    Point pedal(const Point &a) const { return p + v * ((v * (a - p)) / (v * v)); }      // 点在直线上的投影
};
...
struct Convex : Polygon {
    ...
    // 必须保证 is_in(a) == -1
    pair<size_t, size_t> around_points(const Point &a) const {
        const auto &p = this->p;
        if (a == p[0]) return {p.size() - 1, 1};
        if (Segment{p.back(), p[0]}.is_on(a) == 1) return {p.size() - 1, 0};
        if (Segment{p[0], p[1]}.is_on(a) == 1) return {0, 1};
        const auto cmp = [&](const Point &u, const Point &v) { return (u - p[0]).toleft(v - p[0]) == 1; };
        const size_t i = lower_bound(p.begin() + 1, p.end(), a, cmp) - p.begin();
        if (Segment{p[this->pre(i)], p[i]}.is_on(a) == -1) return {pre(i), nxt(i)};
        return {this->pre(i), i};
    }
    // 凸多边形关于某一方向的极点
    // 复杂度 O(logn)
